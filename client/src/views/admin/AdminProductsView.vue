...
function emptyImage(sortOrder = 0) {
...
async function saveType() {
  try {
    if (editingTypeId.value) {
      await adminHttp.patch(`/categories/${editingTypeId.value}`, typeForm.value)
      success('Type updated.')
    } else {
      await adminHttp.post('/categories', typeForm.value)
...
async function disableType(category) {
  try {
    await adminHttp.delete(`/categories/${category.id}`)
...
async function editProduct(product) {
  try {
    const res = await adminHttp.get(`/products/${product.id}`)
    const full = res.data?.data || product
    ...
    productForm.value = {
      ...
      discount_percent:
        Number(full.discount_price) > 0 && Number(full.base_price) > Number(full.discount_price)
          ? round2(((Number(full.base_price) - Number(full.discount_price)) / Number(full.base_price)) * 100)
          : '',
      brand: tags.brand || '',
      ...
      images: Array.isArray(full.images) && full.images.length
        ...
        : [emptyImage(0)],
      variants: Array.isArray(full.variants) && full.variants.length
        ? full.variants.map((v) => ({
          size: v.size || '',
          color: v.color || '',
          sku: v.sku || '',
...
async function saveProduct() {
  ...
  const variants = productForm.value.variants
    .map((variant) => ({
  ...
  const payload = {
    ...
    discount_price: productForm.value.discount_price || null,
    style_tags: productForm.value.style_tags || '',
    images,
    variants,
  }
  ...
  try {
    if (editingProductId.value) {
      await adminHttp.patch(`/products/${editingProductId.value}`, payload)
      success('Product updated.')
    } else {
      await adminHttp.post('/products', payload)
...
async function disableProduct(product) {
  try {
    await adminHttp.delete(`/products/${product.id}`)
...
async function openProductPreview(product) {
  ...
  try {
    const res = await adminHttp.get(`/products/${product.id}`)
    productPreview.value = res.data?.data || null
  } catch (e) {
