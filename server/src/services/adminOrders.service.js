const db = require("../db");
const adminOrdersRepo = require("../repositories/adminOrders.repository");
...
const updateStatus = async (id, status) => {
  ...
  try {
    ...
    const tpl = buildNotificationTemplate("order_status_updated", {
      orderCode: updated?.order_code || current?.order_code,
      nextStatus,
    });
    await safeNotify(() =>
      notificationsService.create({
        sessionId: updated?.session_id || current?.session_id,
        ...
        meta: {
          ...
          nextStatus,
          userEmail: updated?.email || current?.email || null,
