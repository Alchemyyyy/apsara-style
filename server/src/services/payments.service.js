...
function buildBakongStatusRequest({ reference, orderCode = null }) {
  const statusUrl = String(process.env.BAKONG_STATUS_URL || "").trim();
  if (!statusUrl) return null;

  const merchantId = String(process.env.BAKONG_MERCHANT_ID || "APSARA-STYLE").trim();
  const apiKey = String(process.env.BAKONG_API_KEY || "").trim();
  const method = String(process.env.BAKONG_STATUS_METHOD || "GET").trim().toUpperCase();
  ...
  u.searchParams.set("merchant_id", merchantId);
...
const createCheckoutSession = async ({ orderId, sessionId }) => {
  ...
  const merchantId = String(process.env.BAKONG_MERCHANT_ID || "APSARA-STYLE").trim();
  const merchantName = String(process.env.BAKONG_MERCHANT_NAME || "APSARA STYLE").trim();
  const currency = String(process.env.BAKONG_CURRENCY || "USD").toUpperCase();
...
const processBakongWebhook = async ({ headers, body }) => {
  const expectedSecret = String(process.env.BAKONG_WEBHOOK_SECRET || "").trim();
  if (expectedSecret) {
    const provided =
